...
namespace ts {
    ...
    export function transformTypeScript(context: TransformationContext) {
        ...
        function serializeArrayTypeNode(node: any/*ArrayTypeNode*/, expressions?: Expression[]): Expression {
          ...
          let child = (<ArrayTypeNode>node).elementType;
          switch (child.kind) {
            case SyntaxKind.TypeReference:
            ...
            case SyntaxKind.ArrayType:
            expressions.push(createIdentifier("Array"));
              return serializeArrayTypeNode(<ArrayTypeNode>child, expressions);
            default:
              let childNode = serializeTypeNode(child);
              expressions.push(createIdentifier("Array"), childNode);
              return createConstructorExpression(expressions);
            }

        }

        function createConstructorExpression(nodes: (Expression|Identifier)[]): Expression {
          // work in progress
          if (nodes) {
            let constructor: string = "";
            let relativePseudoIndent: string = getIndentString(getIndentSize());
            const singlePseudoIndent: string = "  ";
            for (let i = 1; i <= nodes.length; i++) {
              if ((<any>nodes[i - 1]).text.indexOf('Array') === 0) {
                constructor += "class {";
                relativePseudoIndent += singlePseudoIndent;
                constructor += "\n" + relativePseudoIndent + "constructor(...args) {";
                relativePseudoIndent += singlePseudoIndent;
                constructor += "\n" + relativePseudoIndent + "function fake" + i + "() {}\n" + relativePseudoIndent + "fake" + i + ".prototype = Array.prototype;\n" + relativePseudoIndent + "var obj" + i + " = new fake" + i + "();\n" + relativePseudoIndent + "obj" + i + ".constructor = Array;\n" + relativePseudoIndent + "var newobj" + i + " = Array.apply(obj" + i + ", args);\n" + relativePseudoIndent + "if (newobj" + i + " !== null && (typeof newobj" + i + " === \"object\" || typeof newobj" + i + " === \"function\")) {\n" + relativePseudoIndent + singlePseudoIndent + "obj" + i + " = newobj" + i + ";\n" + relativePseudoIndent + "}\n" + relativePseudoIndent + "Object.defineProperty(obj" + i + ", 'elementType', {\n" + relativePseudoIndent + singlePseudoIndent + "configurable: false,\n" + relativePseudoIndent + singlePseudoIndent + "enumerable: false,\n" + relativePseudoIndent + singlePseudoIndent + "writable: false,\n" + relativePseudoIndent + singlePseudoIndent + "value: ";
                relativePseudoIndent += singlePseudoIndent;
              }
              else {
                constructor += (<any>nodes[i - 1]).text;
              }
            }
            for (let i = (nodes.length - 1); i >= 1; i--) {
              relativePseudoIndent = relativePseudoIndent.substring(0, (relativePseudoIndent.length - 2));
              constructor += "\n" + relativePseudoIndent + "});\n" + relativePseudoIndent + "return obj" + i + ";\n" + relativePseudoIndent.substring(0, (relativePseudoIndent.length - 2)) + "}";
              constructor += "\n" + relativePseudoIndent.substring(0, (relativePseudoIndent.length - 2)) + "static get name() { return Array.name; }\n" + relativePseudoIndent.substring(0, (relativePseudoIndent.length - 2)) + "static get length() { return Array.length; }";
              relativePseudoIndent = relativePseudoIndent.substring(0, (relativePseudoIndent.length - 4));
              constructor += "\n" + relativePseudoIndent + "}"
            }
            let node = createIdentifier(constructor)
            return node;
          }
          else {
            return createIdentifier("Object");
        ...
        function serializeTypeReferenceNode(node: TypeReferenceNode) {
            switch (resolver.getTypeReferenceSerializationKind(node.typeName, currentScope)) {
                ...
                case TypeReferenceSerializationKind.ArrayLikeType:
                    return createIdentifier("Array"); // serializeArrayTypeNode(node, "Array, ");
